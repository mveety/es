...
mkfunpipe(Tree *t1, Tree *t2)
{
	...
	if(t1 == NULL || t1->kind != nList)
		fail("$&parse", "syntax error in function pipe");
	if(t2 == NULL || t2->kind != nList)
		fail("&parse", "syntax error in function pipe");
...
mkmatchall(Tree *subj, Tree *cases)
{
	...
	for(; cases != NULL; cases = cases->CDR) {
		...
		if(!is_wild) {
			...
			match = treecons(mk(nCall, thunkify(treecons(mk(nPrim, "if"),
			...
			wildmatch =
				treecons(mk(nPrim, "if"), treecons(thunkify(matchvar), treecons(wildmatch, NULL)));
			has_wild = 1;
		}
	}
	if(has_wild)
