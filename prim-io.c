...
static List *
redir(const char *caller, List *(*rop)(const char *c, int *fd, List *l), List *list, int evalflags)
{
	int destfd, srcfd;
...
static noreturn
argcount(const char *caller, const char *s)
{
...
static int
pipefork(const char *caller, int p[2], int *extra)
{
	volatile int pid = 0;
