...
extern void
initsignals(Boolean interactive, Boolean allowdumps)
{
	int sig;
	Push settor;
	...
	for(sig = 0; sig < nsignals; sig++)
		if(signals[sig].sig < 1 || NSIG <= signals[sig].sig)
			panic("initsignals: bad signal in sigmsgs.c: %s (see mksignal)", signals[sig].name);
...
{
	return (termeq(e->term, "signal")) && e->next != NULL && termeq(e->next->term, "sigint");
}
